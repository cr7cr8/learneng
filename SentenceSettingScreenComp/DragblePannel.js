        ...
        <View style={[
            {
                ...
                borderBottomWidth: 1, borderBottomColor: "#D2B48C",
            },
            useAnimatedStyle(() => {
                return {
                    //  transform: [{ scale: interpolate(props[1].value, [0, 300], [0, 1], "extend") }]
                    transform: [{ translateX: interpolate(props[1].value, [0, 300], [-300, 0], "extend") }]
                    //opacity:interpolate(props[1].value, [0, 300], [0, 1], "extend")
                }
            })
        ]}>
...
function RightAction({ props, sourceWord, arrIndex, getIndex, panelHeight, newPanelHeight }) {
    ...
    const hashName2 = CryptoJS(sourceWord.exampleEnglishArr[getIndex()].sentence).toString()
    const obj = isDownloadedArr.value.find(item => {
        return (hashName1 + hashName2) === item.sentenceCode
    })
    const isDownloaded = obj?.isDownloaded
    // const isNewAdded = obj?.isNewAdded
            ...
            setSouceWordArr(sourceWordArr => {
                ...
                const arr = sourceWordArr.map(word => {
                    if (sourceWord.wordName !== word.wordName) {
                        ...
                        const newWord = JSON.parse(JSON.stringify(word))
                        const newSentence = `random sentence of ${word.wordName} ${randomNum}`
                        const newHashName2 = CryptoJS(newSentence).toString()

                        newWord.exampleEnglishArr.splice(arrIndex + 1, 0, { "firstTimeAmount": 1, "key": "en-272201", "secondTimeAmount": 1, "sentence": newSentence })
                        ...
                        isDownloadedArr.modify((arr) => {
                            "worklet";
                            arr.push({ sentenceCode: hashName1 + newHashName2, isDownloaded: false, isNewAdded: true })
                            return arr
                        })
    ...
    function deleteSentence() {
        ...
        // isDownloadedArr.modify((arr) => {
        //     "worklet";
        //     const newArr = arr.filter(item => { return item.sentenceCode !== hashName1 + hashName2 })
        //     return newArr
        // })
        setTimeout(() => {
            setSouceWordArr(sourceWordArr => {
    ...
    function setIsDownloadFalse() {
        ...
        isDownloadedArr.modify((arr) => {
            ...
            return [...newArr, { sentenceCode: hashName1 + hashName2, isDownloaded: false }]
    ...
    function setIsDownloadTrue() {
        isDownloadedArr.modify((arr) => {
            ...
            return [...newArr, { sentenceCode: hashName1 + hashName2, isDownloaded: true }]
        })
    }
    ...
    return (
        <View style={[{
            backgroundColor: "#e7cca0", width: 180, height: 80, display: "flex", flexDirection: "row", justifyContent: "center", alignItems: "center",
            borderBottomWidth: 1, borderBottomColor: "#D2B48C",
        },
        useAnimatedStyle(() => {
            return {
            ...
            <GestureDetector gesture={Gesture.Tap()
                ...
                <View style={{ height: 60, width: 60, backgroundColor: "lightblue", justifyContent: "center", alignItems: "center" }}>
                    <Icon
                        ...
                        containerStyle={{ width: 60, height: 60, transform: [{ rotateZ: "0deg" }], backgroundColor: "#e7cca0", justifyContent: "center" }}
                        size={50}
                    />
                </View>
            </GestureDetector>
            ...
            <GestureDetector gesture={Gesture.LongPress()
                .onStart(e => {
                    ...
                    newPanelHeight.value = withTiming(0)
            ...
            <GestureDetector gesture={Gesture.LongPress()
                ...
                useAnimatedStyle(() => {
                    ...
                    <Icon
                        ...
                        containerStyle={{ width: 60, height: 60, transform: [{ rotateZ: "0deg" }], backgroundColor: "wheat", justifyContent: "center" }}
                        size={50}
                    />
                </View>
            </GestureDetector>
            ...
            <GestureDetector gesture={Gesture.LongPress()
                ...
                useAnimatedStyle(() => {
                    ...
                    return {
                        ...
                        transform: [{ scale: isDownloaded ? 1 : 0 }]
                    }
...
export function DragblePannel({ item, drag, isActive, getIndex, allPannelArr }) {
    ...
    const { editorCardY, enText, setEnText, chText, setChText, sentenceIndex, setSentenceIndex, isDownloadedArr } = useContext(DragListContext)
    const { saveWordToFile } = useContext(Context)
    ...
    const newPanelHeight = useSharedValue(0)
    ...
    const customEntering = (targetValues) => {
        ...
        const animations = {
            ...
            // ],
            height: withTiming(80, { duration: 3000 })
        };
        const initialValues = {
            // originX: -screenWidth,
            // opacity: 0,
                                ...
                                onLayout={(e) => {
                                    ...
                                    if (isNewAdded) {
                                        newPanelHeight.value = withTiming(80, { duration: 300 }, () => {
                                            runOnJS(setTimeout)(saveWordToFile, 300)
                                        })
                                    }
                                ...
                                style={[useAnimatedStyle(() => {
                                    ...
                                    return {
                                        ...
                                        borderBottomWidth: isNewAdded
                                            ? newPanelHeight.value === 80 ? 1 : 0
                                            : panelHeight.value === 80 ? 1 : 0,
                                        borderBottomColor: "#D2B48C",
                                        backgroundColor: isDownloaded ? "wheat" : "#e7cca0",
                                        paddingHorizontal: 4,// panelHeight.value === 80 ? 4 : 0,
...
function HeadLeft({ props, sourceWord, item }) {
    ...
    return (
        <View style={[
            {
                backgroundColor: "#e7cca0", width: 300, height: 80 + 30, display: "flex", flexDirection: "row", alignItems: "flex-end",
                borderBottomWidth: 1, borderBottomColor: "#D2B48C",
            },
            useAnimatedStyle(() => {
                return {
            ...
            <GestureDetector gesture={Gesture.Tap().onEnd(e => {
                ...
                <View style={{ height: 60, width: 60, backgroundColor: "lightblue", justifyContent: "center", alignItems: "center" }}>
                    <Icon
                        name="save" type='ionicon' color='orange'
                        containerStyle={{ width: 60, height: 60, transform: [{ rotateZ: "180deg" }], backgroundColor: "#e7cca0", justifyContent: "center" }}
                        size={45}
                    />
                </View>
            </GestureDetector>
...
function HeadRight({ props, sourceWord }) {
    ...
    const { editorCardY, enText, chText, setEnText, setChText, sentenceIndex, setSentenceIndex, isDownloadedArr, newSentenceIndex } = useContext(DragListContext)
    ...
    const isDownloaded = isDownloadedArr.value.find(item => {
        return (hashName1 + hashName2) === item.sentenceCode
    })?.isDownloaded
            ...
            setSouceWordArr(sourceWordArr => {
                ...
                const arr = sourceWordArr.map(word => {
                    if (sourceWord.wordName !== word.wordName) {
                        ...
                        newWord.exampleEnglishArr.splice(0, 0, { "firstTimeAmount": 1, "key": "en-272201", "secondTimeAmount": 1, "sentence": newSentence })
                ...
                FileSystem.readDirectoryAsync(FileSystem.documentDirectory).then(data => {
                    ...
                    data.forEach(item => {
                        if (item.substring(0, 64) === hashName1) {
                            console.log("delete word", sourceWord.wordName, "deleteing looking for its senetence", item.substring(0, 64), item.substring(64, 128))
            ...
            <GestureDetector gesture={Gesture.LongPress()
                ...
                <View style={[{ height: 60, width: 60, backgroundColor: "lightblue", justifyContent: "center", alignItems: "center" }, useAnimatedStyle(() => {
                    ...
                    <Icon
                        name="cloud-download-outline" type='ionicon' color='orange'
                        containerStyle={{
                            width: 60, height: 60, transform: [{ rotateZ: "0deg" }], backgroundColor: "wheat",//"#fcd19dff",
    ...
    return (
        <ReanimatedSwipeable
            ...
            renderRightActions={
                ...
                function (...props) {
                    return (
                        <HeadRight props={props} sourceWord={sourceWord} item={item} />
                    )
                }
            }
            renderLeftActions={
            ...
            <GestureDetector gesture={Gesture.Simultaneous(Gesture.Tap().numberOfTaps(2).onStart(e => {
                ...
                <View
                    style={[useAnimatedStyle(() => {
                        ...
                        return {
                            width: screenWidth, height: 80 + 30, justifyContent: "flex-end", alignItems: "center", borderBottomWidth: 1,
                            borderBottomColor: "#D2B48C",
                            backgroundColor: isDownloaded ? "wheat" : "#e7cca0",
                            marginBottom: 0,//panelMargin.value,
...
export function EditorCard() {
    ...
    function setInfo() {
        setTimeout(() => {
            if (route.params.sentencePos == -1) {
                setSentenceIndex(route.params.sentencePos)
                setEnText(sourceWord.meaning)
                setChText(sourceWord.meaningSound)
            }
            else {
                setSentenceIndex(route.params.sentencePos)
                    ...
                    <View style={useAnimatedStyle(() => {
                        ...
                        <TouchableOpacity activeOpacity={0.2} onPressOut={function () {
                            ...
                            if (sentenceIndex == -1) {
                                ...
                                if (enText == sourceWord.meaning &&
                                    chText == sourceWord.meaningSound) {
                                    ...
                                    setSouceWordArr((sourceWordArr) => {
                                        ...
                                        const arr = sourceWordArr.map(item => {
                                            if (item.wordName !== sourceWord.wordName) {
                                                ...
                                                newSourceWord.meaningSound = chText
