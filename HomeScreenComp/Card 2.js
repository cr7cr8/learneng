...
function RateBar({ index, isDownloaded }) {

    const { setSouceWordArr, saveWordToFile, sourceWordArr, setRefreshState } = useContext(Context)
    const sourceWord = sourceWordArr[index]
    const level = sourceWord.level
    // let realSourceWord = sourceWordArr.find(word => word.wordName === sourceWord.wordName)
    ...
    //return <Text>a</Text>
    //if (isNaN(sourceWord?.level)) { return <></> }
    ...
    return (
        <View style={useAnimatedStyle(() => {
            ...
            return {
                ...
                width: screenWidth, height: 60, flexDirection: "row",
            ...
            {[0, 1, 2, 3, 4, 5].map((level_, index) => {


                return <TouchableOpacity key={Math.random()} activeOpacity={0.2} onPress={function () {
                    setTimeout(() => {
                    ...
                    <View style={
                        ...
                        useAnimatedStyle(() => {
                            return {
                                ...
                                borderWidth: 1, justifyContent: "space-around", alignItems: "center",
                                backgroundColor: level === level_
                                    ? "orange"
                                    : "transparent",
                                padding: 0, margin: 0, paddingHorizontal: 0, marginHorizontal: 0
                        ...
                        <Text style={useAnimatedStyle(() => {

                            return {
                                fontWeight: 900, padding: 0, margin: 0,
                                color: level === level_
                                    ? "wheat"
                                    : "orange"
                            }
...
function WordPanel({ sourceWord, index, visibleCard }) {
    ...
    // useEffect(() => {
    //     setTimeout(() => {
    //         //      wordRowHeight.value = viewRef.current.nativeEvent.layout.height
    //         console.log("====>", sourceWord.wordName, wordRowHeight.value)
    //     }, 100);
        ...
        <SentenceAreaContext.Provider value={{ blockHeightArr }}>
            ...
            >
                <ScrollView
                    ...
                    //   return  <View style={{backgroundColor:"rgba(238, 141, 97, 0.5)",width:screenWidth,height:80,zIndex:999}}></View>
