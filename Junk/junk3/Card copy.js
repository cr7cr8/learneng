...
export default function Card(props, dim) {
    ...
    return (
        <CardContext.Provider value={{ meaningRowHeight_, cardObj }}>
...
function SentenceBlock({ index, arrIndex, children, visibleCard, shouldCheckDownload = true, wordRowHeight, meaningRowHeight, ...props }) {
    ...
    const { sourceWordArr, frameTransY, isCardMoving, downloadWord, refreshState, isScrollingY, setRefreshState,
    ...
    const blockStyle = useAnimatedStyle(() => {
        ...
        if (!blockHeightArr.value.includes(false)) {
            ...
            if (sum <= availableHeight.value) {
                ...
                blockHeight = availableHeight.value * (ratioArr[sentenceIndex])
            }
        }
        return {
            width: screenWidth,
    ...
    return (<>
        ...
        <GestureDetector gesture={Gesture.Pan()
            ...
            .onChange(e=>{
                console.log(e.changeX,e.changeY,sentenceIndex)
            })
            ...
            >
                {/* <ScrollView
                contentContainerStyle={{ width: screenWidth, backgroundColor: "transparent", paddingHorizontal: 4, }}
                onContentSizeChange={(w, h) => { }}
            > */}
...
function SentenceBlock1({ index, arrIndex, children, visibleCard, shouldCheckDownload = true, wordRowHeight, meaningRowHeight, ...props }) {
    ...
    // const { blockHeightArr } = useContext(CardAreaContext)
    const isDownloaded = useSharedValue(false || (!shouldCheckDownload))
    const contentPainText = children.props.children
    //console.log(contentPainText)
    ...
    const sentenceIndex = shouldCheckDownload ? arrIndex * 2 : arrIndex * 2 + 1
    const isLast = sentenceIndex + 1 === cardObj.freeHeightArr.length
    const blockHeight = useSharedValue("auto")
    const isAllMeasured = !cardObj.freeHeightArr.includes(false)

    const heightArr = useSharedValue([])
    //useDerivedValue(()=>{

    if (isAllMeasured) {
        blockHeight.value = cardObj.freeHeightArr[sentenceIndex] //* 2
        const arr = [...cardObj.freeHeightArr]
    ...
    useDerivedValue(() => {
        const a = frameTransY.value;
        if (heightArr.value.length > 0) {

            const sum = heightArr.value.reduce((total, current) => total + current, 0);
            const ratioArr = heightArr.value.map(item => { return item / sum })
            if (sum <= availableHeight.value) {
                blockHeight.value = heightArr.value[sentenceIndex]
            }
            else {
                blockHeight.value = availableHeight.value * (ratioArr[sentenceIndex])
            }


            // console.log(heightArr.value, ratioArr, sourceWord.wordName)


        }
    }, [frameTransY.value, heightArr.value])


    useEffect(() => {
        cardObj.freeHeightArr = cardObj.freeHeightArr.fill(false)
    }, [visibleCard])

    useEffect(() => {
        //  cardObj.freeHeightArr[sentenceIndex] = eref.current?.nativeEvent?.layout?.height + "-----" + sentenceIndex
        setTimeout(() => {

            if (!cardObj.freeHeightArr[sentenceIndex]) {
                cardObj.freeHeightArr[sentenceIndex] = eref.current?.nativeEvent?.layout?.height

                // console.log(eref.current?.nativeEvent?.layout)
                // console.log("+++++++++", sourceWord.wordName, "===", sentenceIndex, "---", cardObj.freeHeightArr)


                if (!cardObj.freeHeightArr.includes(false)) {
                    //  console.log("+++++++++", sourceWord.wordName, "===", sentenceIndex, "---", cardObj.freeHeightArr)
    ...
    const blockStyle = useAnimatedStyle(() => {
        ...
        return {
            ...
            backgroundColor: isDownloaded.value
                ...
                : "#D6BD95",
            backgroundColor: bc,
            opacity: blockHeight.value === "auto" ? 0.3 : withTiming(1),
            elevation: 0,
            marginBottom: 0,
        }
    })
    ...
    return (<>
        ...
        <View style={[blockStyle]}
            ...
            onLayout={(e) => {
                ...
                eref.current = e
                //  cardObj.freeHeightArr[sentenceIndex] = e.nativeEvent.layout.height + "-----" + sentenceIndex

                // setTimeout(() => {
                //                  console.log(     e.nativeEvent.layout.height, sourceWord.wordName + "---" + sentenceIndex)
                // }, 10);
        ...
        >
            <ScrollView
                ...
                onContentSizeChange={(w, h) => {
                    //   console.log(e.nativeEvent.layout.height, sourceWord.wordName + "---" + sentenceIndex)
                    //   if (cardObj.freeHeightArr[sentenceIndex] === "empty") {
                    //   cardObj.freeHeightArr[sentenceIndex] = h + "-----" + sentenceIndex
                    //  }
                    //   console.log(cardObj.freeHeightArr, sourceWord.wordName + "---" + sentenceIndex)
...
function SentenceBlock0({ index, arrIndex, children, visibleCard, shouldCheckDownload = true, wordRowHeight, meaningRowHeight, ...props }) {
    ...
    const { sourceWordArr, frameTransY, isCardMoving, downloadWord, refreshState, isScrollingY,
        isScrollingX, scrollX, speak, isListPlaying, checkPlaying, audioPlayer, stopSpeak } = useContext(Context)
        ...
        >
            ...
            <ScrollView
                ...
                onLayout={(e) => {
                    ...
                    //  isLast && console.log(e.nativeEvent.layout, sentenceIndex)
                }}
            //scrollEnabled={!shouldFramePanning}
            >
...
function getRandomColor() {
    //  "worklet";
    const color = `rgba(${String(Math.floor(256 * Math.random()))},${String(Math.floor(256 * Math.random()))},${String(Math.floor(256 * Math.random()))},0.3)`
