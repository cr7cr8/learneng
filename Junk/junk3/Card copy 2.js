...
function SentenceBlock({ index, arrIndex, children, visibleCard, shouldCheckDownload = true, wordRowHeight, meaningRowHeight, ...props }) {
    ...
    const blockShadowStyle = useAnimatedStyle(() => {
        ...
        // let isSpaceEnough = true
        let isLastSentence = (sentenceIndex + 1) === blockHeightArr.value.length
        const actualHeight = blockHeightArr.value[sentenceIndex] || 40
        ...
        return {
            ...
            transform: [{ scale: actualHeight <= 40 ? 0 : 1 }]
            // transform: [{ translateY: 0 }],
            // zIndex: blockZIndex.value
    ...
    return (<>
        <GestureDetector gesture={Gesture.Tap().numberOfTaps(1)
            .onStart(e => {
            ...
            >
                <View style={[blockShadowStyle]}>
                    <LinearGradient
                        //colors={["rgba(255, 179, 80, 0)", 'rgba(255, 179, 80, 0.8)', 'rgba(255, 155, 133, 1)',]}
                        colors={["transparent", 'rgba(80,80,80,0.25)',]}
                ...
                <ScrollView
                    contentContainerStyle={{ width: screenWidth, backgroundColor: "transparent", paddingHorizontal: 4, elevation: 10 }}
                    onContentSizeChange={(w, h) => { }}
