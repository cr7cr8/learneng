...
function SentenceBlockSpaceEnough({ index, arrIndex, children, visibleCard, shouldCheckDownload = true, wordRowHeight, meaningRowHeight, ...props }) {
    ...
    const blockStyle = useAnimatedStyle(() => {
        ...
        return {
            ...
            // marginBottom: (isLastSentence && !isSpaceEnough.value) ? 80 : 0,
            overflow: "hidden",
            // position: "relative",
            // transform: [{ translateY: 0 }],
            // zIndex: blockZIndex.value
            display: "flex",
    ...
    const [enable, setEnable_] = useState(isSpaceEnough.value)
    ...
    useDerivedValue(() => {
        ...
        //  runOnJS(setEnable)(isSpaceEnough.value)
            ...
            Gesture.LongPress()
                .onStart(e => {
                    ...
                    if (!isDownloaded.value) {
                        ...
                        //runOnJS(setRefreshState)(Math.random())
                    }
...
function SentenceBlock({ index, arrIndex, children, visibleCard, shouldCheckDownload = true, isEnglish = true,wordRowHeight, meaningRowHeight, ...props }) {
    ...
    useEffect(() => {
        ...
        console.log("refrestate", refreshState,"local running")
        const hashName1 = CryptoJS(sourceWord.wordName).toString()
        //     const hashName2 = CryptoJS(shouldCheckDownload ? contentPainText : sourceWord.exampleEnglishArr[arrIndex].sentence).toString()
    ...
    //const bc = getRandomColor(sourceWord.wordName, contentPainText)
    const bc = getBCColor(sentenceIndex % 8)
    // const bbc = getRandomColor()
    ...
    const isSpaceEnough = useSharedValue(true)
    const blockStyle = useAnimatedStyle(() => {
        const downloaded = downloadWord.value
        let blockHeight = "auto"
        // let isSpaceEnough = true
        let isLastSentence = (sentenceIndex + 1) === blockHeightArr.value.length
        ...
        return {
            ...
            //: actualHeight <= 80 ? "#f8dea2ff" : "#D6BD95",


            // backgroundColor: isDownloaded.value ?"wheat":"#DFC98A",
    ...
    const blockShadowStyle = useAnimatedStyle(() => {

        let blockHeight = "auto"
        // let isSpaceEnough = true
        let isLastSentence = (sentenceIndex + 1) === blockHeightArr.value.length
        const actualHeight = blockHeightArr.value[sentenceIndex] || 80
        ...
        if (!blockHeightArr.value.includes(false)) {
            ...
            if (sum <= availableHeight.value) {
                ...
                blockHeight = Math.max(40, availableHeight.value * (ratioArr[sentenceIndex]))
                isSpaceEnough.value = false
            }
        }
        ...
        return {
            ...
            backgroundColor: "transparent",//"rgba(211, 48, 27, 0.3)",
            opacity: 1,//blockHeight === "auto" ? 0 : withTiming(1),
            //opacity: 1,
            ...
            transform: [{ scale: actualHeight <= 80 ? 0 : 1 }]
            // transform: [{ translateY: 0 }],
            // zIndex: blockZIndex.value
    ...
    return (<>
        <GestureDetector gesture={Gesture.Simultaneous(
            ...
            >
                {/* <View style={[blockShadowStyle]}>
                    <LinearGradient
                        //colors={["rgba(255, 179, 80, 0)", 'rgba(255, 179, 80, 0.8)', 'rgba(255, 155, 133, 1)',]}
                        colors={["transparent", 'rgba(80,80,80,0.2)',]}
                        //colors={['rgba(255, 179, 80, 0.8)', 'rgba(255, 155, 133, 1)',]}
                        //colors={['rgba(255, 179, 80, 0.8)', 'rgba(255, 155, 133, 1)',]}
                        dither={true}
                        start={{ x: 0, y: 0 }}
                        end={{ x: 0, y: 1 }}
...
{/*   <LinearGradient
                // colors={["rgba(255, 179, 80, 0)", 'rgba(255, 179, 80, 0.8)', 'rgba(255, 155, 133, 1)',]}
             //   colors={["transparent", 'rgba(100,100,100,0.3)',]}
...
function randomColor() {
    const colorArr = ["#DEB887", "#D2B48C", "#A67B5B", "#DAA520", "#E3BC9A", "#F4A460", "#E3A857", "#C19A6B", "#E6BE8A", "#F5F5DC",
        "#FFDAB9", "#C2B280"]

    return colorArr[Math.floor(colorArr.length * Math.random())]
}
...
function getRandomColor(word1, word2) {
    //  "worklet";
    const rgba = `rgba(${String(Math.floor(256 * Math.random()))},${String(Math.floor(256 * Math.random()))},${String(Math.floor(256 * Math.random()))},0.3)`

    const hashName1 = CryptoJS(word1).toString();
    const hashName2 = CryptoJS(word2).toString();

    //console.log(hashName1, hashName2)
    // console.log("getting random color", rgba)

    return rgba
    return convertToHexValue(extractRGBValues(rgba))
