...
function SentenceBlock({ index, arrIndex, children, visibleCard, shouldCheckDownload = true, wordRowHeight, meaningRowHeight, ...props }) {
    ...
    function expandLast() {
        let sentenceIndex = 999;
        if (!shouldCheckDownload) {
            sentenceIndex = (arrIndex * 2 + 1)
        }
        else {
            sentenceIndex = arrIndex * 2
        }

        if (sentenceIndex === cardObj.freeHeight.length - 1) {
            // console.log(arrIndex, shouldCheckDownload, "@", sourceWord.wordName, cardObj.freeHeight)

            cardObj.freeHeight.forEach(item => {
               if (typeof item === 'number'){
                  if(item>0){
                    console.log("=============",item)
                    maxHeight.value = maxHeight.value + item
                  }
